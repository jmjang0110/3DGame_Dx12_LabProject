...
{
	float fTimeElapsed = m_GameTimer.GetTimeElapsed(); // Delta time ?  
...
{    
	...
	ProcessInput();					// 키,마우스 입력을 받는다 

	AnimateObjects();				// 현재 Scene 의 Objects의 (Scacle , Rotate , Translate ) Transform

    ClearFrameBuffer(RGB(90, 103, 224));		// Buffer Clear ( RGB )

	CCamera* pCamera = m_pPlayer->GetCamera();
	if (m_pScene) m_pScene->Render(m_hDCFrameBuffer, pCamera); // 현재 Scene의 Objects 를 현재 카메라에의 해 찍은 View Space 위치를 계산한 후 그린다.


	PresentFrameBuffer();	// Buffer 에 그린 Resource 를 화면에 출력한디 
}
